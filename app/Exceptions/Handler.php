...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $exception)
    {
        ...
        if ($exception instanceof QueryException) {
            return response()->json(["result" => false, "message" => "Error de consulta BDD ", $exception->getMessage()], 500);
        }

        if ($exception instanceof HttpException) {
            return response()->json(["result" => false, "message" => "Error de ruta"], 404);
        }

        if ($exception instanceof AuthenticationException) {
            return response()->json(["result" => false, "message" => "Error de autenticación"], 401);
        }
