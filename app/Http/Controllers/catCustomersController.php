    ...
    public function customersList(Request $req)
    {
        try {
            ...
            if ($req->has('id_cat_estatus') && !is_null($req->id_cat_estatus)) {
                $query->Where('cat_cliente.id_cat_estatus', '=', $req->id_cat_estatus);
            }
    ...
    public function activeDeactiveDeleteCustomer(ActiveDeactiveDeleteCustomerRequest $req)
    {
        try {
            ...
            if ($req->id_cat_estatus == 3) {
                $updateCustomerstatus->id_usuario_elimina =  $userId;
                $updateCustomerstatus->fecha_eliminacion =   $dateNow;
            } else {
                $updateCustomerstatus->id_usuario_modifica =  $userId;
                $updateCustomerstatus->fecha_modificacion =   $dateNow;
            }
